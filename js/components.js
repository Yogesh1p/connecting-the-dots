...
function initGlobalNavigation(options = {}) {
  ...
  const drawerLinksHTML = NAV_LINKS.map(l => {
    const labelHTML = l.section === 'library' 
      ? `<span class="mobile-library-label">${l.label}</span>` 
      : l.label;
    return `
      <li>
        <a href="${l.href}"
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 5. LIBRARY VISIT MEMORY
  // Mark library page as visited if we are currently in the library section
  if (getCurrentSection() === 'library') {
    try {
      localStorage.setItem('libraryVisited', 'true');
      localStorage.setItem('contentsVisited', 'true');
    } catch (e) {}
  }

  // Also catch clicks to the Library link dynamically
  document.addEventListener('click', (e) => {
...
window.injectGiscusComments = function(containerId) {
  ...
  Object.entries({
    ...
    'data-category-id':'DIC_kwDORe41-s4C7URV',
